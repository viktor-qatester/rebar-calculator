...
**Важно:** не запускайте `python run_bot.py` на ПК, если бот уже работает в облаке — будет конфликт.

### Бот 24/7 в облаке (Render, бесплатно)

Чтобы бот отвечал **без включённого ноутбука** (на стройке, дома у друга):

#### Шаг 1. Регистрация

1. Откройте [render.com](https://render.com) → **Get Started** (можно через GitHub).
2. Подтвердите почту.

#### Шаг 2. Подключить репозиторий

1. В Render: **New +** → **Blueprint**.
2. Подключите GitHub-репозиторий: `viktor-qatester/rebar-calculator`.
3. Render найдёт файл `render.yaml` → **Apply**.

#### Шаг 3. Токен бота

Когда спросит переменные окружения:

| Имя | Значение |
|-----|----------|
| `TELEGRAM_BOT_TOKEN` | ваш токен из `.env` (из BotFather) |

`WEBHOOK_URL` **не нужен** — Render подставит адрес сам.

#### Шаг 4. Дождаться деплоя

- Статус **Live** (зелёный) — обычно 3–5 минут.
- В логах должно быть: `Webhook установлен: https://...onrender.com/webhook`

#### Шаг 5. Проверка

1. **Остановите** `python run_bot.py` на ноутбуке (Ctrl+C), если был запущен.
2. В Telegram: @viktor_context_bot → `/start`.
3. Бот должен ответить без вашего ПК.

#### Если бот молчит

- Render → ваш сервис → **Logs** — скопируйте ошибку.
- Первое сообщение после долгого простоя может идти **до 1 минуты** (бесплатный тариф «просыпается»).
- Проверьте, что токен введён без пробелов.

#### Ссылка для друзей

`https://t.me/viktor_context_bot`
